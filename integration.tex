...
trivial and procedes normally. To allow shared memory interprocess 
communication (IPC) among timing compartments, the shared memory is allocated 
in the address space of the timing compartment with the security label that is 
the greatest lower bound (meet) of the two communicating compartments. Any 
network, cache, or memory controller usage while accessing shared memory uses 
this meet timing compartment. Shared memory cannot be allocated in either of 
the communicating compartments directly. This would allow IPC operations to 
interfere with any other operations performed by that compartment possibly 
causing disallowed timing channels between the communicating timing 
compartments. Often, it may be desirable to create a new timing compartment to 
represent the meet of two communicating compartments rather than to use the 
meet of the two compartments in the original policy.

Message passing IPC is implemented similarly. The timing compartment with the 
label that is the greatest lower bound of each of the communicating 
compartments is used for both sending and receiving messages. An important 
implication of using a TC that is accessed by both communicating parties, is 
that the timing of IPC operations is also visible to both parties. Any programs 
...
explored this topic ~\ref{citation_needed}.

Interdomain communication in Xen is based off of grant tables, a data 
structure that a domain uses to specify the permissions of other domains to use 
its pages. The grant table is shared with Xen, and operated on using 
hypercalls. To provide timing channel protection, grant tables are allocated in 
the meet timing compartment of the communicating domains. The hypercalls must 
be implemented so that their timing does not depend on data that the 
