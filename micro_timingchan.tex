...
Here, we present a classification of internal microarchitectural timing 
...
to another ($E2$) whenever the timing of an action of $E1$ can be affected by 
the behavior of $E2$.
...
In general, the state-based timing channels can be removed by applying 
...
Flushing can remove state-based timing channels for a resource that is 
time-shared.
...
preferable because it allows the full capacity to be used by
...
are
only three main structures that are shared concurrently: a shared cache, 
on-chip
buses, and a memory controller. Moreover, recent studies have proposed 
mechanisms
to control timing channels in each of them. For example, researchers have 
proposed
statically partitioning the shared cache \cite{percival}, and using time 
...
%well-known. Shared caches create a state-based timing channel where 
%interfering entities cause cache block replacements. This can be prevented by 
%partitioning the cache among the entities. On chip networks and buses have
...
However, achieving secure and efficient timing channel protection on a full 
processor
...
%The cache protection scheme also needed to be slightly changed to handle cases 
%when timing
...
solutions.  Then, the next section discusses how these protection mechanisms
...
accesses
from one timing compartment may evict the cache blocks of another timing 
compartment.
...
set partitioning so that each compartment can benefit from full cache 
associativity.
...
interconnects
have contention-based timing channels because each link can be used by only one 
compartment at a time. In our processor design, we apply time
...
block
requests from another. We remove this interference by placing the shared queue 
...
not have a noticeable performance disadvantage over the open page policy, 
because it
...
In this example, we conduct a covert channel attack through cache coherence 
